// Test script for Walk Slot Booking

// This script tests the following scenarios:
// 1. A time slot should remain available until it's fully booked with 4 users
// 2. Once a time slot is fully booked, it should be permanently removed from availability
...
async function runTest() {
  console.log('Starting Walk Slot Booking Test...');
  ...
  try {
    ...
    // Step 3: Book the walk with 3 different users (not fully booked yet)
    console.log('\n--- Step 3: Book the walk with 3 different users ---');
    ...
    // Step 4: Book the walk with the 4th user (should be fully booked now)
    console.log('\n--- Step 4: Book the walk with the 4th user ---');
    const user4Auth = await login(testUsers[3].email, testUsers[3].password);
    
    console.log(`User 4 (${user4Auth.userId}) booking the walk...`);
    await axios.post(
      `${API_URL}/walks/select-walk/${testWalkId}`,
      {
        userId: user4Auth.userId,
        timeSlot: testTimeSlot
      },
      {
        headers: { Authorization: `Bearer ${user4Auth.token}` }
      }
    );
    console.log('User 4 booked successfully');
    
    // Step 5: Verify the time slot is now permanently removed
    console.log('\n--- Step 5: Verify the time slot is permanently removed ---');
    ...
    console.log('\nTest completed successfully! The walk slot booking system is working correctly.');
